...
$hero_image   = travzo_get( 'travzo_faq_hero_image', '' );
$hero_heading = travzo_get( 'travzo_faq_hero_title', 'Frequently Asked Questions' );
$hero_desc    = travzo_get( 'travzo_faq_hero_desc',  'Find answers to the most common questions about travelling with Travzo Holidays.' );
$hero_style   = $hero_image ? 'background-image:url(' . esc_url( $hero_image ) . ');background-size:cover;background-position:center' : '';

// FAQs: cols [0]=category, [1]=question, [2]=answer
$faqs_raw = get_post_meta( $post_id, '_faqs', true );
$faq_data = travzo_parse_lines( $faqs_raw, 3 );

if ( empty( $faq_data ) ) {
    $faq_data = [
        [ 'General',           'What types of travel packages does Travzo offer?',      'Travzo Holidays offers a wide range of travel packages including Group Tours, Honeymoon Packages, Solo Trips, Devotional Tours, Destination Weddings, and International Packages. We cover both domestic Indian destinations and international destinations across Asia, Europe, Africa, and the Americas.' ],
        [ 'General',           'Can I customise a package to suit my preferences?',      'Absolutely. Every package we offer can be fully customised based on your travel dates, budget, accommodation preferences, and specific interests. Simply contact our travel experts and we will craft a personalised itinerary just for you.' ],
        [ 'General',           'How experienced is the Travzo team?',                    'The Travzo team has over 10 years of experience in the travel industry. Our travel experts have firsthand knowledge of hundreds of destinations and have successfully organised thousands of trips for families, couples, groups, and corporate clients.' ],
        [ 'Booking & Payment', 'How do I book a package with Travzo?',                   'You can book a package by filling in the enquiry form on our website, calling us directly, or visiting our nearest branch. Our travel expert will contact you within 24 hours to discuss your requirements and confirm your booking.' ],
        [ 'Booking & Payment', 'What payment methods do you accept?',                    'We accept payments via bank transfer, credit card, debit card, UPI, and net banking. EMI options are available on select packages through partner banks. A booking deposit of 25% is required to confirm your reservation with the balance due 30 days before departure.' ],
        [ 'Booking & Payment', 'Do you offer EMI options?',                              'Yes, we offer EMI options on select packages through our partner banks and credit card companies. Please enquire with our team for the latest EMI offers and eligibility criteria.' ],
        [ 'Visas & Documents', 'Do you help with visa applications?',                    'Yes, we provide complete visa assistance for all international destinations. Our team will guide you through the documentation process, help you prepare your application, and advise on the best time to apply. Please note that visa approval is at the sole discretion of the respective embassy.' ],
        [ 'Visas & Documents', 'What documents do I need for international travel?',     'For international travel you will need a valid passport with at least 6 months validity, the relevant visa, travel insurance, flight tickets, hotel booking confirmations, and sufficient funds. Specific requirements vary by destination and our team will provide a complete checklist for your trip.' ],
        [ 'Visas & Documents', 'How early should I apply for a visa?',                   'We recommend applying for your visa at least 4 to 6 weeks before your departure date. For some destinations like the USA, UK, and Schengen countries, you may need to apply even earlier. Our visa team will advise you on the exact timeline for your destination.' ],
        [ 'Group Tours',       'What is the minimum group size for a group tour?',       'Our group tours typically require a minimum of 10 participants. However, we also offer private group tours for smaller groups of 2 to 9 people at customised pricing. Contact us to discuss your group size and requirements.' ],
        [ 'Group Tours',       'Do you provide a tour manager for group tours?',         'Yes, all our group tours include a dedicated and experienced tour manager who accompanies the group throughout the trip. The tour manager handles all logistics, coordinates with hotels and transport, and ensures a smooth and enjoyable experience for every participant.' ],
        [ 'Honeymoon',         'What makes Travzo honeymoon packages special?',          'Our honeymoon packages are crafted with romance in mind. We include special touches like candlelit dinners, private transfers, flower decorated rooms, and surprise experiences. Every detail is personalised based on the couple\'s preferences to ensure an unforgettable honeymoon.' ],
        [ 'Honeymoon',         'Can I add special surprises to my honeymoon package?',   'Absolutely. We love creating magical moments for honeymooners. You can request flower arrangements, special dinners, spa treatments, photography sessions, or any other special additions. Simply let our team know what you have in mind and we will make it happen.' ],
        ...
        [ 'Cancellation',      'What happens if Travzo cancels my trip?',                'In the unlikely event that Travzo needs to cancel a trip due to unforeseen circumstances, you will receive a full refund or the option to rebook for a future date at no additional cost. We will inform you as early as possible and work with you to find the best solution.' ],
    ];
}

// Map to named keys for the existing template rendering code below
$faq_data   = array_map( fn( $r ) => [ 'faq_category' => $r[0], 'faq_question' => $r[1], 'faq_answer' => $r[2] ], $faq_data );
$categories = array_values( array_unique( array_column( $faq_data, 'faq_category' ) ) );
        ...
        <div class="section-inner">
            <div class="faq-layout">
                ...
                <aside class="faq-sidebar">
                    ...
                    <ul class="faq-category-list" role="list">
                        ...
                        <?php foreach ( $categories as $cat ) :
                            $count    = count( array_filter( $faq_data, fn( $f ) => $f['faq_category'] === $cat ) );
                ...
                <div class="faq-content">
                    ...
                    <?php foreach ( $categories as $cat ) :
                        $cat_slug = sanitize_title( $cat );
                        $cat_faqs = array_values( array_filter( $faq_data, fn( $f ) => $f['faq_category'] === $cat ) );
                    ?>
                    <div class="faq-category-group" data-category="<?php echo esc_attr( $cat_slug ); ?>">
                        ...
                        <div class="faq-accordion">
                            <?php foreach ( $cat_faqs as $faq ) :
                                $q_attr = esc_attr( strtolower( $faq['faq_question'] ) );
                            ?>
